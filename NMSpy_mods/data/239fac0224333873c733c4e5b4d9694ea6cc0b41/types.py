# pyright: reportMissingImports=false
# pyright: reportReturnType=false

# built-in
import ctypes

from typing import Annotated

# pyMHF
from pymhf.core.hooking import function_hook, Structure
from pymhf.core.utils import safe_assign_enum
from pymhf.extensions.cpptypes import std
from pymhf.extensions.ctypes import c_enum32
from pymhf.utils.partial_struct import partial_struct, Field

# NMS.py
from nmspy.data import basic_types as nms_basic_types

# local
from . import enums


# region LanguageManager

@partial_struct
class cTkLanguageManagerBase(Structure):
    _Region: Annotated[enums.eLanguageRegion, Field(c_enum32[enums.eLanguageRegion], offset=0x8)]  # ("meRegion", ctypes.c_int32),

    @property
    def Region(self):
        return safe_assign_enum(enums.eLanguageRegion, self._Region)

    # search for "LANGUAGE\\%s_%s.MBIN" around the offset
    # char *__fastcall sub_1423653E0(__int64 a1, char *a2, char a3)
    @function_hook(signature="48 89 5C 24 10 57 48 81 EC 20 01 00 00 33", offset=0x23653E0)
    def Load(self, this: "ctypes._Pointer[cTkLanguageManagerBase]", arg2: ctypes.c_char_p, arg3: ctypes.c_char_p) -> ctypes.c_char_p:
        pass

# endregion

# region ProductData

@partial_struct
class cGcProductData(Structure):
    # offsets can be taken from https://github.com/monkeyman192/MBINCompiler/blob/v5.29.0-pre1/libMBIN/Source/NMS/GameComponents/GcProductData.cs#L8
    _Description: Annotated[bytes, Field(ctypes.c_char_p, offset=0xF8)]  # ("macDescription", common.cTkDynamicString),
    BaseValue: Annotated[int, Field(ctypes.c_int32, offset=0x16C)]  # ("miBaseValue", ctypes.c_int32),
    _NameLower: Annotated[nms_basic_types.cTkFixedString, Field(nms_basic_types.cTkFixedString[0x80], offset=0x238)]  # ("macNameLower", common.cTkFixedString[0x80]),

    @property
    def Description(self) -> str:
        # sometimes the description is incomplete and needs to be shortened
        try:
            return self._Description.decode()
        except UnicodeDecodeError as e:
            if "position 510" in str(e):
                return self._Description[:-1].decode()
            if "position 509-510" in str(e):
                return self._Description[:-2].decode()

            raise e

    @property
    def NameLower(self) -> str:
        return str(self._NameLower).strip()

# endregion

# region RealityManager / Technology / Stats

@partial_struct
class cGcStatsTypes(Structure):
    # offsets can be taken from https://github.com/monkeyman192/MBINCompiler/blob/v5.29.0-pre1/libMBIN/Source/NMS/GameComponents/GcStatsTypes.cs#L4
    _StatsType: Annotated[enums.eStatsType, Field(c_enum32[enums.eStatsType], offset=0x0)]  # ("_meStatsType", ctypes.c_uint32),

    @property
    def StatsType(self):
        return safe_assign_enum(enums.eStatsType, self._StatsType)


# must contain all fields as it is used in an array
@partial_struct
class cGcStatsBonus(Structure):
    # offsets can be taken from https://github.com/monkeyman192/MBINCompiler/blob/v5.29.0-pre1/libMBIN/Source/NMS/GameComponents/GcStatsBonus.cs#L6
    Bonus: Annotated[float, Field(ctypes.c_float, offset=0x0)]  # ("mfBonus", ctypes.c_float),
    Level: Annotated[int, Field(ctypes.c_int32, offset=0x4)]  # ("miLevel", ctypes.c_int32),
    Stat: Annotated[cGcStatsTypes, Field(cGcStatsTypes, offset=0x8)]  # ("mStat", cGcStatsTypes),


@partial_struct
class cGcTechnology(Structure):
    # offsets can be taken from https://github.com/monkeyman192/MBINCompiler/blob/v5.29.0-pre1/libMBIN/Source/NMS/GameComponents/GcTechnology.cs#L8
    StatBonuses: Annotated[list[cGcStatsBonus], Field(nms_basic_types.cTkDynamicArray[cGcStatsBonus], offset=0x158)]  # ("maStatBonuses", common.cTkDynamicArray[cGcStatsBonus]),
    _NameLower: Annotated[nms_basic_types.cTkFixedString, Field(nms_basic_types.cTkFixedString[0x80], offset=0x244)]  # ("macNameLower", common.cTkFixedString[0x80]),

    @property
    def NameLower(self) -> str:
        return str(self._NameLower).strip()


@partial_struct
class cGcRealityManager(Structure):
    PendingNewTechnologies: Annotated[list[int], Field(std.vector[ctypes._Pointer[cGcTechnology]], offset=0x258)]  # ("PendingNewTechnologies", std.vector[ctypes.POINTER(cGcTechnology)]),

    # search for "Metadata/Simulation/Missions/Tables/MissionTable.mXml" around the offset
    # __int64 __fastcall sub_140D14080(__int64 *a1)
    @function_hook(signature="48 89 4C 24 08 55 53 56 57 41 54 41 56 41 57 48 8D AC 24 E0", offset=0x0D14080)
    def Construct(self, this: "ctypes._Pointer[cGcRealityManager]") -> ctypes.c_uint64:
        pass

    # __int64 __fastcall sub_140D20C40(__int64 a1, __int64 a2, char *a3)
    @function_hook(signature="48 89 5C 24 08 45 0F", offset=0x0D20C40)
    def GetHashedIDForTech(self, this: "ctypes._Pointer[cGcRealityManager]", result: ctypes.c_char_p, lTechID: ctypes.c_char_p) -> ctypes.c_char_p:
        pass

    # search for "ITEMGEN_FORMAT_FREI_PASS" around the offset
    # __int64 __fastcall sub_140D218B0(_QWORD *a1, __int128 *a2)
    @function_hook(signature="48 89 54 24 10 48 89 4C 24 08 55 53 41 54 48", offset=0x0D218B0)
    def GenerateProceduralProduct(self, this: "ctypes._Pointer[cGcRealityManager]", lProcProdID: ctypes.c_char_p) -> ctypes.c_uint64:  # ctypes._Pointer[cGcProductData]:
        pass

    # search for "UI_WIKI_PROC_TECH_SUB" around the offset
    # __int64 __fastcall sub_140D24F40(__int64 a1, __int128 *a2, char a3)
    @function_hook(signature="44 88 44 24 18 48 89 4C 24 08 55 41", offset=0x0D24F40)
    def GenerateProceduralTechnology(self, this: "ctypes._Pointer[cGcRealityManager]", lProcTechID: ctypes.c_char_p, lbExampleForWiki: ctypes.c_bool) -> ctypes.c_uint64:  # ctypes._Pointer[cGcTechnology]:
        pass

# endregion
